...
class Dataset(Dataset):
    def __init__(
        ...
        self.transform_aug = T.Compose([
            T.Resize([image_size,image_size]),
            T.RandomHorizontalFlip() if if_flip else nn.Identity(),
            T.RandomCrop(image_size) if if_crop else nn.Identity(),
            T.RandomRotation(90) if if_rot else nn.Identity(),
            T.RandomAffine(degrees=0,shear=45) if if_shear else nn.Identity(),
            Cutout() if if_cutout else nn.Identity(),
            T.ToTensor(),
            T.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
        ])
    ...
    def __getitem__(self, index):
        ...
        if image_label==3 or image_label==5 or image_label==6:
            return (self.transform_aug(image), image_label)
        else:
...
def train(args):
    ...
    #transform=get_transform(aug_type,image_size)
    ...
    print(device)
    print(if_flip,if_crop,if_rot,if_shear,if_cutout)
    ...
    for fold in range(k_fold):
        ...
        ds = Dataset(image_folder, image_size, train_split,anno_fold,if_flip,if_crop,if_rot,if_shear,if_cutout)
        ...
        #torch.save(class_net.state_dict(), os.path.join(save_fold,'last','classifier_fold_'+str(fold)+'_'+aug_type+'_test.pth'))
        ...
        for e in range(epochs):
            ...
            if e>80 and loss<min_loss:
                min_loss=loss
                torch.save(class_net.state_dict(), os.path.join(save_fold,'best','classifier_fold_'+str(fold)+'_'+aug_type+'_best.pth'))
            batch_bar.set_postfix({'Loss': np.mean(losses)})
            losses = []
            print('lr: ', scheduler.get_last_lr()[0])
            scheduler.step()
        torch.save(class_net.state_dict(), os.path.join(save_fold,'last','classifier_fold_'+str(fold)+'_'+aug_type+'_last.pth'))
        print('Saved classifier model: 0')

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='')
    parser.add_argument('--device', type=str, default='cuda:5', help='device')
    parser.add_argument('--aug_type', type=str, default='flip', help='augmentation type:flip,crop,rot,shear,cutout')
    ...
    parser.add_argument('--image_fold', type=str, default='/ISIC2018_Task3_Training_Input', help='train images')
    parser.add_argument('--anno_fold', type=str, default='/ISIC_2018_5fold2', help='5fold annotation fold')
